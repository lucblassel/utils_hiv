    ...
    for param, spec in params_file.items():
        if spec[0]:
            ...
            if distrib not in ALLOWED_DISTRIBS.keys():
                raise ValueError('Allowed distributions: {}'
                                 .format(", ".join(ALLOWED_DISTRIBS.keys()))
                                 )
            else:
                param_distribs[param] = ALLOWED_DISTRIBS[distrib]\
                    (low, high)
        else:
            param_distribs[param] = spec[1:]

    return list(ParameterSampler(param_distribs, num_iter)) if param_distribs != {} else {}
