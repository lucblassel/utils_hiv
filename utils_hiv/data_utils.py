...
def deep_clean_features(dataset):
    ...
    """
    features_to_clean = dataset.filter(regex=r"\d+_(nan|-|\*|X)", axis=1).columns
...
def subsampling_balance(data, target):
    ...
    balanced = pd.concat([min_df, maj_df.sample(n=min_n, replace=False)]).sample(
        frac=1
    )  # shuffling data
...
def choose_subtype(dataset_path, metadata_path, subtype):
    ...
    index = metadata[
        metadata.filter(regex=r".*(s|S)ubtype.*", axis=1).iloc[:, 0] == subtype
    ].index
...
def get_parameter_cv_folds(dataset_path, num_folds, num_repeats, target, balance=False):
    ...
    splitter = RepeatedStratifiedKFold(n_splits=num_folds, n_repeats=num_repeats)
...
def resample_binary_task(data, final_pos_freq=0.5, target="encoded_label"):
    ...
    neg_count, pos_count = (
        round((1 - final_pos_freq) * final_N),
        round(final_pos_freq * final_N),
    )
